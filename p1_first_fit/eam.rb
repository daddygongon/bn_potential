...
#    system.atoms.each{|atom| p atom}
  ...
  def mk_nl
    @system.atoms[0 .. (@n_atoms - 1)].each_with_index do |iatom, i|
      @system.atoms[0 .. -1].each_with_index do |jatom, j|
        next if j <= i
        # p [distance(iatom.pos, jatom.pos), @cut_off]
  ...
  def eam_param_select(poq='al2.89')
    ...
    # rev for al
    unless poq == 'jindo' or poq == 'anharm' or poq == 'al' or poq == ''
      #      @cut_off = 4.0414 * 0.82 # for EAM 1st
            @cut_off = 3.626 * 0.6 # for EAM 1st
    end
  end
  ...
  def total_energy()
    @system.atoms.inject(0.0) do |sum, atom|
